...
-- Department 1 has two employees (needed for the one-to-many test)
...
-- Employee 1 has skills 1 (Java) and 3 (SQL) -- NOT 2 (Python), so the
-- add-skill test can add skill 2.
...
-- ==== Stock (Hands-on 2) -- curated sample of real Yahoo Finance values ====
-- FB, September 2019 (the 19 rows expected by the date-between query)
...
-- FB outside September (must be excluded by the between query)
...
-- FB highest-volume days (top-3 by volume)
...
-- GOOGL: closing price greater than 1250
